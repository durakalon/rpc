...
def solve_for_k_vehicles(vehicle: Vehicle, items: List[Item], k: int, time_limit: float, verbose: bool = False) -> Optional[List[Placement]]:
    ...
    if verbose:
        print(f"DEBUG: {lifo_constraints_count} LIFO constraints added", file=sys.stderr)
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        # DEBUG: Vérifier les contraintes LIFO après résolution
        if verbose:
            print(f"DEBUG: Checking LIFO constraints in solution...", file=sys.stderr)
            violations = 0
            for i in range(n):
                for j in range(i + 1, n):
                    if items[i].delivery_time != -1 and items[j].delivery_time != -1:
                        if result[i].vehicle_id == result[j].vehicle_id:  # Même véhicule
                            xi, xj = result[i].x, result[j].x
                            di, dj = items[i].delivery_time, items[j].delivery_time
                            if di < dj and xi < xj:
                                violations += 1
                                print(f"  VIOLATION: item {i} (d={di}, x={xi}) should be >= item {j} (d={dj}, x={xj})", file=sys.stderr)
                            elif dj < di and xj < xi:
                                violations += 1
                                print(f"  VIOLATION: item {j} (d={dj}, x={xj}) should be >= item {i} (d={di}, x={xi})", file=sys.stderr)
            print(f"DEBUG: {violations} LIFO violations found", file=sys.stderr)
